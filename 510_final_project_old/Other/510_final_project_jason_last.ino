...
static double robX;
static double robY;
static double robViveX;
static double robViveY;
static double duty;
static double deltX;
static double deltY;
static double diffX;
...
// Input can arguments in vive coordinates (if necessary) convert to RHR 19 coordinates
void setCan(double viveCanX, double viveCanY) {
  canX = viveCanX / 2000.0;
...
//  velY = 0;
  ...
  // Input these vive coordinate XY locations if known i.e. provided by a TA
  setCan(3690, 5480);
...
void updateVals() {
  ...
  // @param swap (above false) indicates whether Vive z-axis is downward IRL
...
void updateRobotXY() {
  ...
  robViveX = 0;
  int countXY = 0;
  while(robViveY == 0 && robViveX == 0) {
    if (countXY > 0) { Serial.println("Vive reading (0, 0)"); }
    robViveX = vive1.xCoord();
    robViveY = vive1.yCoord();
    countXY += 1;
  }
  robX = robViveX / 2000.0;
